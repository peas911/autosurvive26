...
extern uint8_t usart3_rx_buf[2][SBUS_RX_BUF_NUM];
...
void USART3_IRQHandler(void)
{
  ...
  if (huart3.Instance->SR & UART_FLAG_IDLE)
  {
      ...
      if ((huart3.hdmarx->Instance->CR & DMA_SxCR_CT) == RESET)
      {
          ...
          this_time_rx_len = SBUS_RX_BUF_NUM - huart3.hdmarx->Instance->NDTR;
          ...
          {
              sbus_to_rc(usart3_rx_buf[0], &rc_ctrl);
          }
      }
      else
      {
          ...
          if (this_time_rx_len == RC_FRAME_LENGTH)
          {
              sbus_to_rc(usart3_rx_buf[1], &rc_ctrl);
          }
      }
  }
